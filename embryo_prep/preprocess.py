...
def train_data(train_generator, validation_generator, n_train_steps, n_validation_steps, overwrite,
               ...
               save_best_only, labmda_ac, beta_ac, gamma_ac, theta_ac, denoising, depth, drop_xy_levels, mask_shape, t, concat_mask):
    ...
    if not overwrite and len(glob.glob(model_file + '*.hdf5')) > 0:
        ...
        model = load_old_model(model_path, loss, initial_learning_rate, dropout_rate, input_shape, chosen_model, lambda_ac=labmda_ac,
                               beta_ac=beta_ac, gamma_ac=gamma_ac, theta_ac=theta_ac, denoising=denoising)
    else:
...
def generate_data(scans_dir, data_dir, normalization, training_modalities, ext, overwrite, batch_size,data_split_test, data_split_validation, validation_file, training_file, test_file,
                  ...
                  patches_per_epoch, is_3D, truth_index, truth_size, truth_crop, categorical, preprocess, scale, rescale_res, metadata_path):
    ...
    data_file_opened = create_load_hdf5(normalization=normalization, data_dir=data_dir, scans_dir=scans_dir, train_modalities=training_modalities, ext=ext,
                                        overwrite=overwrite, preprocess=preprocess, scale=scale, rescale_res=rescale_res, metadata_path=metadata_path)
